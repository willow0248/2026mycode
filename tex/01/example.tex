...
\section{问题的背景}
\begin{enumerate}
  \item \textbf{平面波假设：}入射红外光为理想平面单色波，忽略光束发散效应；
  \item \textbf{均匀薄膜假设：}外延层厚度 $d$ 在测量区域内均匀一致，
        界面为理想平行平面；
  \item \textbf{各向同性假设：}外延层和衬底均为各向同性介质，
        折射率不随偏振方向变化；
  \item \textbf{色散模型假设：}外延层折射率 $n(\tilde{\nu})$ 满足
        Sellmeier 色散关系（具体形式见第4节）；
  \item \textbf{问题一特定假设：}仅考虑外延层上下界面各一次反射，
        忽略多次反射的贡献（此假设在问题三中予以放宽）。
\end{enumerate}
